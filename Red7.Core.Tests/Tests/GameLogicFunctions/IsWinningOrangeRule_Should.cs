...
namespace Red7.Core.Tests.Tests.GameLogicFunctions
{
    ...
    public class IsWinningOrangeRule_Should
    {
        ...
        public void Return_False_If_Opponent_Has_More_Cards_Matching_Rule()
        {
            ...
            _opponentPalettes.Add(new Palette(2)
            {
                Cards = new List<Card>()
                {
                    ...
                    new Card() { Color = Color.Yellow, Value = 3 },
                }
            });

            // Act
            var isWinning = GameLogic.IsWinningOrangeRule(_activePlayerPalette, _opponentPalettes);
